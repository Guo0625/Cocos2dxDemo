...
wb.PushAdBtn = ccui.Widget.extend({
    ...
    setData: function (data) {
        ...
        wb.loadImg(this.data.icon, function (texture2d) {
    ...
    createBtnSp: function (texture2d) {
        ...
        if (!this.clippingNode) {
            isFirst = true;
            var stencil = new cc.Sprite(wb.pushRes.pushFrame);
            ...
            this.bgSp = new cc.Sprite(wb.pushRes.pushFrame);
    ...
    onClick: function () {
        ...
        if (openType == "image") {
            ...
            // if (this.data.openPath && this.data.openPath.length > 0) {
            //     wb.Browser.openInnerWeb(this.data.openPath);
            // }
            wb.navigateToMiniProgram(this.data.linkUrl,this.data.icon);
    ...
    // 展示数上报
    showPost: function () {
        wb.navigateToShow(this.data.linkUrl,this.data.icon)
        //var tjData = {
        //    label: this.data.id,
