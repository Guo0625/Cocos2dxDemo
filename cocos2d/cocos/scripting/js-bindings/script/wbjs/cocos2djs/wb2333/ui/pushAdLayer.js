...
wb.PushAdLayer = cc.Layer.extend({
    ...
    check: function (layer) {
        var bottomList = wb.pushData.bottomList;
    ...
    initUI: function (pData, layer, size, btnIndexList) {
        // var bottomList = wb.pushData.bottomList;
        var bottomList = wb.data23PushData.adlist;
...
wb.createPushAdLayer = function (placement, btnIndexList, num, btnDw) {
   ...
   // var bottomList = wb.pushData.bottomList;
    ...
    // var bg = new cc.Sprite(wb.pushRes.wxPushBg);
    var bg = new cc.Node();
    bg.setContentSize(cc.size(510, 150));
    bg.setAnchorPoint(cc.p(0.5, 0.5));
    var bgSize = cc.size(455, 155);

    var bgCenterP = cc.p(bgSize.width / 2, bgSize.height / 2 + 10);
    ...
    for (var i = 0; i < pList.length; i++) {
        var frameSp = new cc.Sprite(wb.pushRes.pushFrame);
